...
public class WeixinServiceImpl implements WeixinService {
    ...
    public Map<String, Object> createNative(Long orderId) {
        ...
        try {
            ...
            client.post();
            //返回相关数据
            var xml = client.getContent();
            //转换成map集合
            var resultMap = WXPayUtil.xmlToMap(xml);
  ...
  //微信退款
    ...
    public Boolean refund(Long orderId) {
        //获取支付记录信息
        var paymentInfo = paymentService.getPaymentInfo(orderId, PaymentTypeEnum.WEIXIN.getStatus());
//        if(paymentInfo==null) return false;
        //添加信息到退款记录表中
        var refundInfo = refundInfoService.saveRefundInfo(paymentInfo);
        //判断当前订单数据是否已经退款
